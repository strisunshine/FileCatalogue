...
void FileCatalogue::find(const string& path){
	...
	for (auto patt : _patterns){
		...
		for (auto f : files){
			...
			if (it == _datastore.end() || (it != _datastore.end() && !_datastore.findPathIter(f, fullpath)))
				_datastore.save(f, fullpath);
		}
	}
}
...
void FileCatalogue::findText(const string& text, const std::string& fullfilespec){
	DisplayHelper DH;
	...
	if (fi.isGood()){
		...
		if (content.find(transtext) != string::npos) DH.displayFullpath(fullfilespec);
	}
	fi.close();
}
